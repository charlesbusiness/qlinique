<x-app-layout>
    ...
    @if ($treatment->category === 'maternal_health' && $treatment->maternalHealthRecord)
        @include('treatments.show-maternal', ['record' => $treatment->maternalHealthRecord, 'treatment' => $treatment])
    @else
                    ...
                    <div class="card-body">
                        ...
                        <p class="mb-1"><strong>Category:</strong> <span class="badge bg-warning">{{ \App\Enums\TreatmentCategory::tryFrom($treatment->category)?->label() ?? ucfirst($treatment->category) }}</span>
                            ...
                            @elseif ($treatment->sub_category)
                                — {{ (\App\Livewire\TreatmentForm::assessmentSubOptions($treatment->category))[$treatment->sub_category] ?? $treatment->sub_category }}
                            @endif
                        </p>
